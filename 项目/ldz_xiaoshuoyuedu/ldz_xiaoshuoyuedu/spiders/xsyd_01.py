...
class Xsyd01Spider(scrapy.Spider):
    name = 'xsyd_01'
    allowed_domains = ['readnovel.com']
    start_urls = ['https://www.readnovel.com/all']

    def parse(self, response):
        data_lis = response.xpath('//ul[@type="category"]/li[@data-id!="-1"]')
        # print(len(data_lis))
        for data_li in data_lis:
            req_url = response.urljoin(data_li.xpath('./a/@href').extract_first(""))
            yield scrapy.Request(req_url,callback=self.one_parse)
